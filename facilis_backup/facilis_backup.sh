...
SCN_COMMERCIAL_BACKUP_DIR="/Volumes/blackhole/facilis_backup/scn_commercial"
WBD_PROMO_BACKUP_DIR="/Volumes/blackhole/facilis_backup/wbd_promo"
...
send_email_report() {
    ...
    if command -v mail >/dev/null 2>&1; then
        if mail -s "$subject" "$EMAIL_TO" < "$RUN_LOG_FILE"; then
...
EOF
    ...
    trap '[[ -n "${RUN_LOG_FILE:-}" && -f "$RUN_LOG_FILE" ]] && rm -f "$RUN_LOG_FILE"' EXIT
...
## Add the dry-run to rsync, use a tmp variable for this!
    ...
    if ! command -v fccmd &> /dev/null; then
    ...
    local mounts
    mounts=$(fccmd list_mounts 2>/dev/null | awk -F' = ' -v mount_base="/Volumes/" '
    ...
    while IFS='|' read -r _ volume_name volume_capacity volume_available; do
        ...
        if [[ "$volume_available" != "unknown" ]]; then
            ...
            if [[ "$available_gb" -ge 0 && "$available_gb" -lt 500 ]]; then
    ...
    while IFS='|' read -r volume volume_name volume_capacity volume_available; do
        ...
        if mkdir -p "$backup_path"; then
            ...
            if [[ "$DEBUG_RSYNC" == "1" ]]; then
                ...
                if rsync $RSYNC_OPTS --stats $exclude_opts "$volume/" "$backup_path/" 2>&1 | tee "$rsync_tmp_log" | tee -a "$DEBUG_LOG_FILE"; then
                    rsync_exit_code=0
                else
                    rsync_exit_code=$?
                fi
                ...
                if rsync_output=$(rsync $RSYNC_OPTS --stats $exclude_opts "$volume/" "$backup_path/" 2>&1); then
                    rsync_exit_code=0
                else
                    rsync_exit_code=$?
                fi
